...
* [x] **Generate `README.md`** — If `README.md` does not exist at the project root, create it using the following structure:
  ...
  * Brief description: A Spring Boot service that consumes messages from an Apache Artemis queue (`weather.queue`), exposes a REST API to query ordered historical/current weather by city and date, and persists successful requests/responses to PostgreSQL.
